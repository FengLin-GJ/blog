//网站最后更新时间（版本更新需更改）
var siteLastUpdateTime = '2021年5月1日18点';

//网站开始时间
var siteBeginRunningTime = '2020-12-12 20:00:00';
...
function reputInArticle(data, value) {
    $('.articles01').empty();
    var articles = $('.articles01');
    $.each(data, function (index, obj) {
        if (index < (data.length) - 2) {
            var title = "";
            if (value != "") {
                var re = new RegExp("\(" + value + "\)", "gi");
                title = obj['articleTitle'].replace(re, "<span style='color:#ff0000'>$1</span>");
            } else {
            ...
            var center = $('<div class="center">' +
                ...
                '<h1 itemprop="name">' +
                '<a class="article-title" href="' + obj['thisArticleUrl'] + '" target="_blank">' + title + '</a>' +
                '</h1>' +
                '<div class="article-meta row" id="bug_">' +
                ...
                '</div>' +
                '<hr>' +
                '<div class="article-tags01">' +
                '</div>' +
                '</div>');
            articles.append(center);
            var articleTags = $('.article-tags01');
            ...
            $('#badge_').empty();
            var badge_ = $('#badge_');
            var center1 = $('<li role="presentation"><a href="#">推荐标签&nbsp;/&nbsp;推荐系数</a></li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
            badge_.append(center1);
            for (var key in obj) {
                if (obj[key] != 0) {
                    var center = $('<li role="presentation"><a href="#">' + key + ' <span class="badge">' + obj[key] + '</span></a></li>');
                    badge_.append(center);
                }
            }
        }
    })
...
//填充文章
function putInArticle(data, value) {
    $('.articles').empty();
...
//搜索
function search(currentPage, name, value) {
    if (value != "") {
        searchArticles(currentPage, name, value);
    } else {
        ajaxFirst(1);
    }
}
...
//搜索文章
function searchArticles(currentPage, name, value) {
    //加载时请求
    $.ajax({
        type: 'GET',
        url: '/search',
...
//文章推荐
function reArticles(currentPage) {
    var name = $('.username_').text();
    //加载时请求
    $.ajax({
        type: 'GET',
        url: '/recommend',
...
$('.myCardBtn').click(function () {
    $('.qq_code').attr("src", "https://test-bucket-s.oss-cn-zhangjiakou.aliyuncs.com/Image/qq_code.jpg");
    $('.weixin_code').attr("src", "https://test-bucket-s.oss-cn-zhangjiakou.aliyuncs.com/Image/weixin_code.jpg");
});
...
$.ajax({
    ...
    success: function (data) {
        if (data['status'] == 103) {
            dangerNotice(data['message'] + " 获取公告信息失败");
        } else {
...
$.ajax({
    ...
    success: function (data) {
        if (data['status'] == 103) {
            ...
            if (JSON.stringify(data['data']) != '{}') {
                $.each(data['data'], function (index, obj) {
                    $('.img-words small').html(obj);
                    $('.personal-card-show-words small').html(obj);
                })
            }
        }
    },
    ...
    success: function (data) {
        if (data['status'] == 103) {
            ...
            if (JSON.stringify(data['data']) != '{}') {
                $('.roll ul').empty();
                $.each(data['data'], function (index, obj) {
                    var message_ = $('.roll ul');
                    var center = $('<li>' + obj + '</li>');
