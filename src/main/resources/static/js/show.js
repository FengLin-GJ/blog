...
function putInArticle(data) {
    $('.GanJ-article-top').html('');
    $('.GanJ-article-footer').html('');
    var zym = getPersonal();
    if (zym == "") {
        zym = "多年以后，愿你的城市，有清风，有烈酒，也有人是你的归途。";
    }
    var articleTop = $('<article-top><div class="article-title">' +
        ...
        zym + '<span class="article-i-say-me">--- GanJ</span>' +
        '</div>');
    $('.GanJ-article-top').append(articleTop);
    ...
    var articleFooter = $('<div class="end-logo">' +
        ...
        '<img src="https://test-bucket-s.oss-cn-zhangjiakou.aliyuncs.com/Image/weixinPublic">' +
        '</p>' +
        '<p class="show-weixin-pic">欢迎关注我的微信公众号：GanJ</p>' +
        ...
        '</span>' +
        '</div>');
    $('.GanJ-article-footer').append(articleFooter);
    ...
    $('.other').append($('<div class="social-share" data-initialized="true" data-url="https://www.ganjie./article/' + data.articleId + '"  data-title="' + data.articleTitle + '">' +
...
$.ajax({
    ...
    success: function (data) {
        if (data['status'] == 0) {
            ...
            $('.content').html('');
            var error = $('<div class="article"><div class="GanJ-article-top"><div class="error">' +
