...
$('.superAdminList .superAdminClick').click(function () {
    ...
    $('#statistics,#articleManagement,#articleThumbsUp,#articleCategories,#friendLink,#announcement,#userFeedback,#privateWord').css("display", "none");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data['status'] == 601) {
                    ...
                    //刷新刚填充上的链接的两个按钮，使编辑和删除两个按钮的js生效
                    updateFriendLinkEditAndDelBtn();
                } else if (data['status'] == 103) {
                    dangerNotice(data['message'] + " 更新链接失败")
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data['status'] == 604) {
                ...
                dangerNotice(data['message'] + " 删除链接失败")
            } else {
                dangerNotice(data['message']);
            }
        },
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data['data']['result'].length == 0) {
                if ($('.privateWord').html() == null || $('.privateWord').length == 0) {
...
//点击公告
$('.superAdminList .announcement').click(function () {
...
$('.superAdminList .friendLink').click(function () {
    $.ajax({
        ...
        success: function (data) {
            if (data['status'] == 103) {
                dangerNotice(data['message'] + " 获得链接失败")
            } else {
...
function modify_(type, key) {
    var s = $('.' + key).text();
    var person = prompt("请输入新内容！", s);
    if (person != null && person != "" && person != s) {
        $('.' + key).html(person);
        modifyMessage_(type, key, person);
    }
}

function modifyMessage_(type, key, value) {
    $.ajax({
        type: 'POST',
        url: '/modifyMessage',
        dataType: 'json',
        ...
        success: function (data) {
            if (data['status'] == 103) {
                dangerNotice(data['message'] + "修改公告信息失败");
            } else {
...
function delete_(type, key, class_) {
    $.ajax({
        type: 'POST',
        url: '/deleteMessage',
        dataType: 'json',
        ...
        success: function (data) {
            if (data['status'] == 103) {
                dangerNotice(data['message'] + " 删除公告信息失败");
            } else {
                $('.' + class_).empty();
                getMessage_(type, class_);
            }
        },
        error: function () {
            alert("删除公告信息失败！");
...
function add_(type, class_) {
    var person = prompt("请输入新内容！", "");
    if (person != null && person != "") {
        addMessage_(type, person, class_);
    }
}

function addMessage_(type, value, class_) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data['status'] == 103) {
                dangerNotice(data['message'] + " 添加公告信息失败");
            } else {
                $('.' + class_).empty();
                getMessage_(type, class_);
            }
...
function getMessage_(type, class_) {
    //加载时请求
    $.ajax({
        type: 'POST',
        url: '/getMessage',
        ...
        success: function (data) {
            if (data['status'] == 103) {
                dangerNotice(data['message'] + " 获得公告信息失败");
            } else if (JSON.stringify(data['data']) != '{}') {
                if ($.trim(class_) == $.trim("lunboMessage_")) {
                    $.each(data['data'], function (index, obj) {
                        addLunbo(type, index, obj);
                    })
                } else if ($.trim(class_) == $.trim("personalMessage_")) {
                    $.each(data['data'], function (index, obj) {
                        addPersonal(type, index, obj);
                    })
                    $('.add_icon02').empty();
                } else {
                    addSystem(data['data']);
                ...
                nullMessage_(class_);
                add_icon02();
            } else if ($.trim(class_) == $.trim("systemMessage_")) {
...
function nullMessage_(class_) {
    $('.' + class_).empty();
    var message_ = $('.' + class_);
    var center = $('<p>暂时还没有公告信息，马上添加信息！！！</p>');
    message_.append(center);
}

function add_icon02() {
    var message = $('.add_icon02');
    var center = $('<img src="img/add_.svg" onclick="add_(\'personal\',\'personalMessage_\')"/>');
    message.append(center);
}

function addLunbo(type, key, value) {
    var message_ = $('.lunboMessage_');
    var center = $('<div class="lunbo_"><p class="' + key + '">' + value + '</p>\n' +
        '&nbsp;&nbsp;\n' +
        '<div class="lunbo_icon">\n' +
        '<img src="img/delete_.svg" onclick="delete_(\'' + type + '\',\'' + key + '\',\'lunboMessage_\')"/>\n' +
        '&nbsp;&nbsp;\n' +
        '<img src="img/modify_.svg" onclick="modify_(\'' + type + '\',\'' + key + '\')"/>\n' +
        '</div>\n' +
        '</div><br>');
    message_.append(center);
}

function addPersonal(type, key, value) {
    var po = $('.po');
    po.html(value);
    var message = $('.personalMessage_');
    var center = $('<small class="' + key + '" ">' + value + '</small>\n' +
        '<div class="personal_icon">\n' +
        '<img src="img/delete_.svg" onclick="delete_(\'' + type + '\',\'' + key + '\',\'personalMessage_\')"/>\n' +
        '&nbsp;&nbsp;\n' +
        '<img src="img/modify_.svg" onclick="modify_(\'' + type + '\',\'' + key + '\')"/>\n' +
        '</div>');
    message.append(center);
...
function loadingMessage_() {
    getMessage_("lunbo", "lunboMessage_");
    getMessage_("personal", "personalMessage_");
    getMessage_("system", "systemMessage_");
}
